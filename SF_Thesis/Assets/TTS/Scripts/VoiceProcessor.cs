 ...
 * 2021-02-19
 * Modified by Ian Zamora Bella, 2025
 * 
 * Apache License
 * 
...
public class VoiceProcessor : MonoBehaviour
{
    ...
    public event Action<short[]> OnFrameCapturedRaw;       // raw mic frames
    public event Action<short[]> OnFrameCapturedEnhanced;  // AGC frames for STT
    ...
    public void UpdateDevices()
    {
        ...
        // Clamp index for builds (device ordering/count can differ)
        CurrentDeviceIndex = Mathf.Clamp(MicrophoneIndex, 0, Devices.Count - 1);

        Debug.Log($"[VoiceProcessor] Using mic '{CurrentDeviceName}' (index {CurrentDeviceIndex}/{Devices.Count})");
    }
    ...
    IEnumerator RecordData()
    {
        ...
        while (IsRecording)
        {
            ...
            if (_audioDetected)
            {
                ...
                // raw path (monster should use this!)
                short[] rawPcm = new short[sampleBuffer.Length];
                for (int i = 0; i < FrameLength; i++)
                    rawPcm[i] = (short)(Mathf.Clamp(sampleBuffer[i], -1f, 1f) * short.MaxValue);

                if (_transmit)
                    OnFrameCapturedRaw?.Invoke(rawPcm);

                // Enhanced path (stt should use this!)
                float[] enhanced = new float[sampleBuffer.Length];
                Array.Copy(sampleBuffer, enhanced, sampleBuffer.Length);

                
                float rms = 0f;
                for (int i = 0; i < enhanced.Length; i++)
                    rms += enhanced[i] * enhanced[i];
                rms = Mathf.Sqrt(rms / enhanced.Length);

                float gate = 0.0035f; 
                if (rms >= gate)
                {
                    float targetRms = 0.08f; 
                    float maxGain = 12f;     
                    float gain = Mathf.Clamp(targetRms / (rms + 1e-8f), 1f, maxGain);

                    for (int i = 0; i < enhanced.Length; i++)
                        enhanced[i] = Mathf.Clamp(enhanced[i] * gain, -1f, 1f);
                }

                short[] enhancedPcm = new short[enhanced.Length];
                for (int i = 0; i < FrameLength; i++)
                    enhancedPcm[i] = (short)(enhanced[i] * short.MaxValue);

                if (_transmit)
                    OnFrameCapturedEnhanced?.Invoke(enhancedPcm);
